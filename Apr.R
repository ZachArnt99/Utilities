library(dplyr)
balances <- data.frame(Rob = 595.94, Jesse = 499, Luis1 = 0, Luis2 = 0,
                       Luis3 = 0, Zach = 0, Mike = 1095)

#Housemates only pay for utilities on dates when they sleep at the house. The first step is to create a "presence" dataframe.

P <- read.table("marApr.csv", header = T, sep = ",") #P stands for presence. The .csv was created by exporting the most recently completed (or nearly completed) two full calendar months of the raw attendance record from Google Sheets.
dates <- P[,1]
P <- P[,2:9]
row.names(P) <- dates
P
...
U_bill <- 137.32 #Enter the amount of the Longmont utility bill for water and electricity.
U_first <- which(row.names(P) == "Mar 6") #Enter the billing date date of the second most recent utility bill. Use the three letter abbreviation for the month, followed by a space, followed by one or two digits for the day.
U_last <- which(row.names(P) == "Apr 5") #Enter the day prior to the most most recent utility billing date.
#Do the same for the gas and internet bills
G_bill <- 41.98
G_first <- which(row.names(P) == "Mar 21")
G_last <- which(row.names(P) == "Apr 20")
I_bill <- 49.95
I_first <- which(row.names(P) == "Mar 20")
I_last <- which(row.names(P) == "Apr 19")
...
Rent_first <- which(row.names(P) == "Apr 1")
Rent_last <- which(row.names(P) == "Apr 30")
...
na_days <- data.frame(Rob = 0, Jesse = 0, Luis1 = 19, Luis2 = 19, Luis3 = 19, Zach = 0, Mike = 26, Basil = 30) 
...
Tots <- rbind(Utot,Gtot,Itot,base_Rent,Rent_adjustment) |> round(2)
row.names(Tots) <- c("Util","Gas","Inet","Rent","Adj")
sum_Tots <- Tots |> colSums() |> round(2)
Tots[5,2] <- 685.73/30*-2/2
Tots
sum_Tots
