...
P <- read.table("P.csv", header = T, sep = ",") #P stands for presence. P.csv was created by exporting the most recently completed (or nearly completed) two full calendar months of the raw attendance record from Google Sheets.
dates <- P[,1]
P <- P[,2:9]
row.names(P) <- dates
...
Rent_adjustment <- -1*days_away/2*base_Rent/days_Rent
